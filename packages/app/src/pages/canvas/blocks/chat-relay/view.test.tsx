...
describe("ChatRelayBody", () => {
  test("renders permission denial from project policy or host access", () => {
    ...
    expect(policy.host.querySelector(".canvas-session-surface")).toBeNull()
    policy.dispose()
  ...
  test("mounts the native session surface for the host-owned binding", () => {
    runtimeHandle = handle("ready", {
      workspaceID: "ws-1",
      sessionID: "session-1",
      directory: "/workspace",
    ...
    const mounted = mount(props())
    const surface = mounted.host.querySelector(".canvas-session-surface")
  ...
  test("preserves the last valid native surface while refresh is stale or transiently failing", () => {
    const view = {
      workspaceID: "ws-1",
      sessionID: "session-1",
      directory: "/workspace",
    ...
    for (const status of ["stale", "error"] as const) {
      ...
      const mounted = mount(props())
      expect(mounted.host.querySelector(".canvas-session-surface")?.getAttribute("data-session-id")).toBe("session-1")
      mounted.dispose()
    }
  })
})
