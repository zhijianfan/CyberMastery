...
const compact = Effect.fnUntraced(function* (
  ...
  yield* events.publish(
    ...
    { commit: () => SessionCompactionContext.write(db, messageID, context) },
  )
  return context
})
    ...
    Effect.gen(function* () {
      ...
      yield* freshTarget(
        Effect.gen(function* () {
          ...
          expect(yield* SessionCompactionContext.read(db, source.compactionID)).toEqual(source.compaction)
          expect(
            yield* db
              .select()
    ...
    Effect.gen(function* () {
      ...
      yield* freshTarget(
        Effect.gen(function* () {
          ...
          yield* insertProject(db)
          yield* transfer.restoreBatch({ bundle: source.bundle })
          const conflicting = yield* SessionContextSidecar.render({
