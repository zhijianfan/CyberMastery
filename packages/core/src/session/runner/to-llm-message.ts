...
function toLLMMessage(
  ...
  compactionContexts: ReadonlyMap<SessionMessage.ID, SessionCompactionContext.V1>,
): Message[] {
  switch (message.type) {
    case "agent-switched":
    case "model-switched":
      return []
    case "user":
      const snapshot = contextSnapshots.get(message.id)
      return [
        Message.make({
          id: message.id,
          role: "user",
          content: [
            { type: "text", text: snapshot?.version === 2 ? snapshot.apiContent : message.text },
    ...
    case "compaction":
      const checkpoint = compactionContexts.get(message.id)
      return [
        Message.make({
          id: message.id,
          role: "user",
...
export const toLLMMessages = (
  ...
  model: Model,
  contextSnapshots: ReadonlyMap<SessionMessage.ID, SessionContextSnapshot> = new Map(),
  compactionContexts: ReadonlyMap<SessionMessage.ID, SessionCompactionContext.V1> = new Map(),
) => messages.flatMap((message) => toLLMMessage(message, model, contextSnapshots, compactionContexts))
