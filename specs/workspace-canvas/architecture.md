...
`environment` (preset + functionality availability) and `style` (visual/density
preference) are distinct dimensions per ADR-2.
...
3. Migrate the current client-side workspace store (directories/plugins/
   environment) to host hydration; keep `Persist` as cache.
4. Introduce editing mode + layout persistence last, on the stable shell.

Landed on the branch so far: the client-side workspace/environment store
